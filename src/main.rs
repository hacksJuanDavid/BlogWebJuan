...
fn query_select_all_carousel() {
    ...
    let mut conn = conectar_bd(); //Conectamos a la base de datos
    //Select * from posts
...
fn query_select_all_post() {
    ...
    //Select * from posts
    let posts_result = posts.order(id).load::<Post>(&mut conn).expect("Error al ejecutar la query");
...
fn query_insert_carousel(
    ...
    let _carousel: Carousel = diesel
        ::insert_into(carousel::table)
        .values(&new_carousel)
        .get_result(&mut conn)
...
fn query_insert_symbol(
    ...
    let _symbol: Symbol = diesel
        ::insert_into(symbol::table)
        .values(new_symbol)
        .get_result(&mut conn)
...
fn query_insert_p_index(
    ...
    let _p_index: Pindex = diesel
        ::insert_into(pindex::table)
        .values(new_p_index)
        .get_result(&mut conn)
        .expect("No se ha podido establecer los datos.");
...
fn query_insert_p_service(
    ...
    let _p_service: Pservice = diesel
        ::insert_into(pservice::table)
...
fn query_insert_layout_service(
    ...
    let _layout_service: LayoutService = diesel
        ::insert_into(layoutservice::table)
...
fn query_insert_layout_profile(
    ...
    let _layout_profile: LayoutProfile = diesel
        ::insert_into(layoutprofile::table)
...
fn query_insert_p_profile(
    ...
    let _p_profile: Pprofile = diesel
        ::insert_into(pprofile::table)
...
fn query_insert_layout_post(
    ...
    let _layout_post: LayoutPost = diesel
        ::insert_into(layoutpost::table)
...
fn query_insert_p_contact(
    ...
    let _p_contact: PContact = diesel
        ::insert_into(p_contact::table)
...
fn query_insert_social(
    ...
    let _social: Social = diesel
        ::insert_into(socials::table)
        .values(&new_social)
        .get_result(&mut conn)
...
fn query_insert_msg(
    ...
    let _msg: Msg = diesel
        ::insert_into(msg::table)
        .values(&new_msg)
        .get_result(&mut conn)
        .expect("Error al insertar el msg");

    let msg_result = msg.limit(1).load::<Msg>(&mut conn).expect("Error al ejecutar la query");
    for _msg in msg_result {
        println!(
            "
        First Name:{}
...
async fn index(pool: web::Data<DbPool>, template_manager: web::Data<tera::Tera>) -> impl Responder {
    ...
    match web::block(move || {carousel.load::<Carousel>(&mut conn)}).await {
        Ok(data) => {
...
async fn post(pool: web::Data<DbPool>, template_manager: web::Data<tera::Tera>) -> impl Responder {
    let mut conn = pool.get().expect("Problemas al traer la base de datos");
    let diesel_capture_dates_1 = posts.load::<Post>(&mut conn).expect("Error al ejecutar la query");
...
async fn contact_post(data: web::Form<Msg>) -> impl Responder {
    ...
    query_insert_msg(&data.first_name, &data.last_name, &data.email, &data.country, &data.message);
